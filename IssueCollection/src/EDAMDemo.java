...
public class EDAMDemo {
	...
	private static final String authToken = "S=s1:U=6bba5:E=14652f8aa39:C=13efb477e3c:P=1cd:A=en-devtoken:V=2:H=078eb4f922d28fac46894da7d3c4e0a7";
	...
	public static void main(String args[]) throws Exception {
		...
		if (demo.intitialize()) {
			try {
				List<IssueData> data= Parser.ParserMethod();
				long start=0;
				long time=0;
				
				while(true){
					start=System.currentTimeMillis();	
					demo.createNote(data);	
					if ((time = System.currentTimeMillis() - start) < 60000) {
...
//				demo.updateNoteTag();
	 ...
	 */
	private void createNote(List<IssueData> data) throws Exception {
		// To create a new note, simply create a new Note object and fill in
		// attributes such as the note's title.
		SimpleDateFormat sf=new SimpleDateFormat("yyyy-MM-dd-hh-mm");
		
		Note note = new Note();
		note.setTitle(sf.format(new Date(System.currentTimeMillis())));
...
//				 "<en-media type=\"image/png\" hash=\"" + hashHex + "\"/><en-media type=\"image/png\" hash=\"" + hashHex + "\"/><br/>"
				
				;
				int count=0;
		for(IssueData i : data)
		{

			body.append("<span style=\"color:green;\"><![CDATA[keyword:"+i.getKeyword()+"]]></span><br/>");
			
			String[] url=i.getImageurl().split(";");
			...
			if(url.length>0&&!url[0].equals(""))
			{
				for(String s:url)
				{
					...
					resource.setData(readWebImageAsData(s));
					resource.setMime(mimeType);
					ResourceAttributes attributes = new ResourceAttributes();
					attributes.setFileName(""+(count++));
					resource.setAttributes(attributes);
					note.addToResources(resource);
					String hashHex = bytesToHex(resource.getData().getBodyHash());	
					body.append("<en-media type=\"image/png\" hash=\"" + hashHex + "\"/>");
//					body.append("<span style=\"color:black;\">Keyword:"+s+"</span><br/>");
				}
				body.append("<br/>");
			}
			body.append("<span style=\"color:black;\"><![CDATA[content:"+i.getContent()+"]]></span><br/>");
			body.append("<a target=\"_blank\" href=\""+i.getLink().replaceAll("&", "&amp;")+"\" shape=\"rect\" >Link</a><br/><br/>");
		}
...
//						+ "<en-media type=\"image/png\" hash=\"" + hashHex + "\"/><en-media type=\"image/png\" hash=\"" + hashHex + "\"/><br/>"
	...
	private void searchNotes() throws Exception {
		...
		while (iter.hasNext()) {
			...
			Note fullNote = noteStore.getNote(authToken, note.getGuid(), true,
					true, false, false);
	...
	private void updateNoteTag() throws Exception {
		...
		// tag.
		Note note = noteStore.getNote(authToken, newNoteGuid, true, true,
				false, false);
		...
		note = noteStore.getNote(authToken, newNoteGuid, false, false, false,
				false);
	...
	private static Data readWebImageAsData(String fileName) throws Exception {
